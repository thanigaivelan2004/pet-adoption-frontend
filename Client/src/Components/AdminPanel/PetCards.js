...
const PetCards = (props) => {
  ...
  const handleApprove = async () => {
    setIsApproving(true);
    try {
      const response = await fetch(`https://pet-adoption-backend-hl87.onrender.com/approving/${props.pet._id}`, {
        method: 'PUT',
        body: JSON.stringify({
          status: "Approved"
        }),
  ...
  const deleteFormsAdoptedPet = async () => {
    setIsDeleting(true)
    try {
      const deleteResponses = await fetch(`https://pet-adoption-backend-hl87.onrender.com/form/delete/many/${props.pet._id}`, {
        method: 'DELETE'
      });
  ...
  const handleReject = async () => {
    try {
      const response = await fetch(`https://pet-adoption-backend-hl87.onrender.com/delete/${props.pet._id}`, {
        method: 'DELETE'
      })
    ...
    <div className='req-containter'>
      <div className='pet-view-card'>
        <div className='pet-card-pic'>
          <img src={`https://pet-adoption-backend-hl87.onrender.com/images/${props.pet.filename}`} alt={props.pet.name} />
        </div>
