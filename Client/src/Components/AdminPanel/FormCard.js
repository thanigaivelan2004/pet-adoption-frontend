...
const FormCard = (props) => {
  ...
  const handleApprove = async () => {
    setIsApproving(true);
    try {
      const response = await fetch(`https://pet-adoption-backend-hl87.onrender.com/approving/${props.form.petId}`, {
        method: 'PUT',
        body: JSON.stringify({
  ...
  const deleteFormAdoptedPet = async () => {
    try {
      const deleteResponse = await fetch(`https://pet-adoption-backend-hl87.onrender.com/form/delete/many/${props.form.petId}`, {
        method: 'DELETE'
      });
  ...
  const handleReject = async () => {
    setIsDeleting(true)
    try {
      const response = await fetch(`https://pet-adoption-backend-hl87.onrender.com/form/reject/${props.form._id}`, {
        method: 'DELETE'
      })
